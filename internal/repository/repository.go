...
func (p *PostgresRepository) SaveUser(ctx context.Context, username string, email string, passHash []byte) (uid int64, err error) {
	...
	var id int
	err = p.DB.DB.QueryRow("INSERT INTO user (username, email, hash_password) VALUES($1, $2, $3) RETURNING id", username, email, passHash).Scan(&id)
	if err != nil {
		var pgErr *pq.Error
...
func (p *PostgresRepository) User(ctx context.Context, email string) (models.User, error) {
	...
	err := p.DB.DB.QueryRow("SELECT id, username, email, hash_password FROM user WHERE email=$1", email).Scan(&user.ID, &user.Username, &user.Email, &user.HashPassword)
	if err != nil {
...
func (p *PostgresRepository) UserID(ctx context.Context, username string) (int64, error) {
	...
	var id int64
	err := p.DB.DB.QueryRow("SELECT id FROM user WHERE username = $1", username).Scan(&id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			p.log.Error("user not found", "error", ErrUserNotFound)
